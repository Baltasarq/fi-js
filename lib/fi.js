...
const LocTraits = (function() {   
    ...
    function isLocListingObjectsInside()
    {       
        return this.locListObjectsInside;
    }
    
    function setLocListsObjectsInside(v)
    {
        if ( arguments.length === 0 ) {
            v = true;
        }
    ...
    return {
        // showObjectsInside
        "isLocListingObjectsInside": isLocListingObjectsInside,
        "setLocListsObjectsInside": setLocListsObjectsInside,
        // doEachTurn
...
LocTraits.locListObjectsInside = true;
...
const ctrl = ( function() {
    ...
    function updateObjects()
    {
        ...
        const inventory = ctrl.personas.getPlayer().objs;
        const reachable = ctrl.places.getCurrentLoc().objs;
        const personas = ctrl.places.getCurrentLoc().personas;
        const pObjects = document.createElement( "p" );
        let strEntitiesList = "";
        const objRefPrefix = "<a class=\""
                            + HtmlClassRef.links.obj
                            + "\" onclick=\"javascript:ctrl.addTerm('";
        const pnjRefPrefix = "<a class=\""
    ...
    */
    ...
    function list(loc)
    {
          ...
          || loc.isLocListingObjectsInside() )
        {
            ...
            const portableObjs = loc.objs.filter( x => !x.isScenery() );

            // List them
            let totalObjsListed = 0;
            for(const obj of portableObjs) {
                toret += "${" + obj.id + ',';
                ...
                // Delimiter
                if ( totalObjsListed == ( portableObjs.length - 2 ) ) {
                    toret += " y ";
                }
                else
                if ( totalObjsListed < ( portableObjs.length - 2 ) ) {
