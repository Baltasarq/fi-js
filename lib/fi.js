...
const LocTraits = (function() {   
    ...
    /** Creates an scenery object and returns it.
      * @param name the name of the object, the printable one.
      * @param syns a list with those synonym words.
      * @param desc the description of the object.
      * @return an scenery obj.
      */
    function creaScenery(name, syns, desc)
    {
        return ctrl.creaObj( name, syns, desc, this, Ent.Scenery );
    }
    ...
    return {
        ...
        // Object creation in here
        "creaScenery": creaScenery,
        "creaEscenario": creaScenery,
        // Pre- and post- actions
...
const ctrl = ( function() {
    ...
    */
    ...
    function callInis()
    {
        let pendingEnts = new Set();
        let initedEnts = new Set();
        
        // Add all locations and its objects
        ctrl.locs.locs.forEach( loc => {
            pendingEnts.add( loc );
            loc.objs.forEach( obj => pendingEnts.add( obj ) );
        });
        
        // Add all personas and its objects
        ctrl.personas.personas.forEach( persona => {
            pendingEnts.add( persona );
            persona.objs.forEach( obj => pendingEnts.add( obj ) );
        });

        do {
            let newPendingEnts = new Set();
            
            // Store the inited ents (they will be)
            pendingEnts.forEach( ent => initedEnts.add( ent ) );
            ...
            // Prepare new pending entities
            pendingEnts.clear();
            newPendingEnts.forEach( e => {
                if ( !initedEnts.has( e ) ) {
                    pendingEnts.add( e );
                }
            });
        } while ( pendingEnts.size > 0 );
